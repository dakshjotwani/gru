...
|---|---|---|
| **Event Source** | Emit session events to minions | Hooks (shell scripts) → HTTP POST |
...
- Knowledge accumulation
- Chat interface
- Session launching (you still launch `claude` manually; minions just watches)

**Bootstrap value:** Once Phase 1 is running, you can use minions to monitor the Claude Code sessions that are building Phase 2.
...
4. **Session state persistence:** If minions backend restarts, can it reconstruct session state from hooks that fire after restart, or do we need to persist enough to survive restarts?
