...
End-to-end browser testing surfaced four security-relevant gaps between this spec and what actually works:

**PDF iframe drops `sandbox` entirely; same-origin deployments downgrade to a download card.** The design specifies `<iframe sandbox="" src="…">` for PDFs to neutralize embedded JS. In Chrome, this leaves the tab blank — the built-in PDF viewer ships its UI as a chrome-extension that needs same-origin scripting to bootstrap, and any sandbox value (empty *or* `allow-scripts`) prevents that bootstrap. The implementation drops the `sandbox` attribute for PDF specifically.

The defense for PDF therefore reduces to: server-side `%PDF-` magic-byte check (so it really is a PDF, not malicious HTML), `Content-Type: application/pdf` plus `X-Content-Type-Options: nosniff` (so the browser can't reinterpret it as HTML), and the artifact endpoint running on a *different* origin from the dashboard (so embedded PDF JS sits in a separate same-origin-policy bucket from the dashboard's cookies and DOM).

This last condition is fragile — Gru's prod deploy mounts the SPA on the same port as the API (`cmd/gru/server.go`), so iframe and parent are same-origin. To avoid silently regressing the threat model, the frontend now compares `new URL(artifact.url).origin` to `window.location.origin` and *downgrades to a download card* in same-origin deployments. PDF inline preview is therefore only rendered when the dev/Vite split puts the dashboard and the artifact server on different ports; a single-origin operator gets a "Open PDF in a new tab" card. Markdown rendering is unaffected (its iframe is `sandbox=""` regardless of origin).
...
**`POST /artifacts` has no Bearer auth.** The design spelled out `Authorization: Bearer <api-key>` on the upload endpoint matching the gRPC API. Implementation drops this — `internal/config` no longer carries an `api_key` field at all (rationale per the existing comment: *"bearer auth removed; tailnet bind is the boundary"*). The `/events` ingestion endpoint had already made the same call. New posture: every HTTP-level write surface (events, artifact upload, link add via gRPC if you count Connect-RPC POSTs) trusts the network bind. Acceptable for Gru's local-/tailnet-only deployment. Threat: an in-browser script the operator visits can issue cross-origin POSTs against `localhost:7777` and create artifact rows on a session whose UUID it knows or can guess; UUIDv4 makes guessing impractical, but the design's earlier "bearer is in another origin's bundle and not exposed via cookie" framing only protected the bearer-protected gRPC surface and didn't apply here. Document, don't re-add bearer.

**No `artifact.deleted` / `session_link.deleted` events.** The design publishes `*.created` events on every successful upload but the implementation does not publish corresponding `*.deleted` events from the gRPC delete handlers. Practical consequence: a second browser tab open on the same session will not see the deleted artifact disappear until it remounts and refetches `ListArtifacts`. Acceptable for MVP — single-operator, one-tab-typical — but flagged because it breaks the symmetry the rest of the design implies. Adding `*.deleted` events is a small follow-up: publish from `Manager.Delete` and from each gRPC delete handler with the deleted id as payload, mirror the create-event reducer in `useSessionArtifacts.ts`.
...
- **2026-04-25 (after pre-merge code review)** — Expanded §5's "Implementation deviations" subsection with two more honesties: (1) the un-sandboxed PDF iframe collapses to a download card in same-origin deployments, since Gru's prod deploy mounts the SPA and the API on the same port and the design's "different origin gives natural isolation" only holds in dev split-port mode; (2) `POST /artifacts` does not carry Bearer auth — `internal/config` removed the api_key field (matching what `/events` already did), and the threat-model framing earlier in §5 only protected the bearer-protected gRPC surface, not the HTTP write endpoints; documenting rather than re-adding bearer because the deployment posture is local-/tailnet-only. Also flagged an unimplemented part of the design: `artifact.deleted` / `session_link.deleted` events are not published from the delete paths, so cross-tab live updates on deletion don't work — a follow-up task, not a re-design.
