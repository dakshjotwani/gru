...
**MVP:** Pre-shared API key configured in `~/.gru/config.yaml`. Sent as gRPC metadata (`authorization: Bearer <key>`) on all RPCs. Hook scripts include it via environment variable (`MINIONS_API_KEY`). Event ingestion endpoint (HTTP POST from hooks) uses the same key in the `Authorization` header.
